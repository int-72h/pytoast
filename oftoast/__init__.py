...
def download_file_multi(arr):
    ...
    if not local:
        ...
        u = open(Path(url) / "/of_toast" / filename,'rb')
...
def download_db(path,local=False):
    if not local:
        ...
        f = open(Path(url) / of_toast / "oftoast.json",'r')
        memfile = f.read()
        f.close()
...
def argvparse():
    ...
    global nproc
    uhelp = """\1
Usage: oftoast -p . [-u (default server url)] [-n 4]
...
def main():
    ...
    try:
        ...
        font = ("Staatliches Regular",20)
    except:
        font = ("Liberation Sans",20)
    print(font)
    guiparse()
    todl = []
    dbs = download_db(prefix,True)
    ...
    if nproc > 1:
        try:
            z = [(z[0],z[1],True) for z in todl]
            res = [x for x in pbar_sg(z)]
        except ImportError:
            nproc = 1
    if nproc <= 1:
        sbar_sg(todl)
